#!groovy
    ...
    def branch
	def projectName = 'mylibrary-rental-service'
    ...
    node(javaAgent) {
        ...
        try {
			...
			stage('Dockerize') {
				...
				sh "sudo docker run --restart always --name=${projectName} -e CONFIG_SERVER_URI=http://localhost:8888 -p 8083:8083 -td ${projectName}"
			}
        } catch (def e) {
